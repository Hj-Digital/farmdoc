...
export default function Admin() {
  ...
  if (!confirm("Tem certeza que deseja excluir TODOS os arquivos?")) return;

  const res = await fetch('/api/limpar', { method: 'DELETE' });
  const data = await res.json();

  alert(data.message || data.error);
  fetchArquivos(); // atualiza lista
};
      ...
      <div style={containerStyle}>
        {!acesso ? (
          ...
          <div>
            ...
            <button onClick={limparArquivos} style={{ ...styles.button, marginBottom: 20, background: '#dc2626' }}>
